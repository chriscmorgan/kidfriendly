...
export const metadata: Metadata = {
  title: 'Kid-Friendly Cafes in Melbourne with Play Areas',
  description: 'Discover the best cafes in Melbourne with kids play areas — indoor playgrounds, spots next to parks, and family-friendly restaurants reviewed by local parents.',
  alternates: { canonical: `${SITE_URL}/melbourne` },
  openGraph: {
    title: 'Kid-Friendly Cafes in Melbourne with Play Areas | KidFriendlyEats',
    description: 'Find cafes, restaurants and venues in Melbourne where kids can play — reviewed by local parents.',
    url: `${SITE_URL}/melbourne`,
  },
}

async function getMelbourneLocations(): Promise<Location[]> {
  if (USE_MOCK) {
...
export default async function MelbournePage() {
  const locations = await getMelbourneLocations()
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: 'Melbourne', item: `${SITE_URL}/melbourne` },
    ],
  }
  ...
  const itemListSchema = locations.length > 0
    ? {
        '@context': 'https://schema.org',
        '@type': 'ItemList',
        name: 'Kid-Friendly Cafes in Melbourne',
        description: 'Cafes and venues in Melbourne with play areas for kids',
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex flex-wrap gap-2">
            {[
              ...
              'Northcote', 'Prahran', 'Collingwood', 'Carlton', 'Hawthorn',
              'Camberwell', 'Williamstown', 'Footscray', 'Docklands', 'Elwood',
            ].map((suburb) => (
              <Link
                key={suburb}
