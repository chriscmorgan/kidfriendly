  ...
  return (
    <button
      ...
      className={cn(
        ...
        active ? 'bg-rust text-paper border-transparent' : 'bg-paper text-ink border-border shadow-sm'
      )}
    >
      {label}
    </button>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs text-stone mt-0.5 flex items-center gap-1 truncate">
          ...
          {loc.distance_km != null && (
            <span className="text-rust font-medium ml-1">{loc.distance_km.toFixed(1)}km</span>
          )}
        </p>
      </div>
      {rating > 0 && (
      ...
      <div className="flex-1 overflow-y-auto overscroll-contain divide-y divide-gray-50">
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <p className="font-semibold text-ink text-sm mb-1">No spots found here yet</p>
            <p className="text-xs text-stone mb-5">Be the first to add a cafe or spot with a play area here.</p>
            <a
              href="/submit"
        ...
        {showSearchArea && (
          <div className="absolute z-20 left-1/2 -translate-x-1/2" style={{ top: '68px' }}>
            <button
              ...
              className="flex items-center gap-1.5 bg-paper text-ink text-xs font-semibold px-4 py-2 rounded-full shadow-lg border border-border cursor-pointer hover:bg-parchment transition-colors whitespace-nowrap"
            >
